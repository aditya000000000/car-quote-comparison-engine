...
export default function QuotesPage() {
  ...
  // Compare feature
  const [compareList, setCompareList] = useState([]); // array of quoteIds
  const [showCompareModal, setShowCompareModal] = useState(false);

  // Sort + Filters
  const [sortBy, setSortBy] = useState("lowPremium"); // lowPremium | highCSR | highCashless
  ...
  // Recommended plan (highest CSR among current list)
  ...
  // View breakup modal is driven by URL param (NO state-in-effect)
  ...
  const onAIRecommend = async (quote) => {
    try {
      setAiQuote(quote);
      setAiOpen(true);
      setAiLoading(true);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50 px-4 py-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="w-full">
              ...
              <input
                ...
                placeholder="Search insurer or plan name (e.g. HDFC, Premium)..."
        ...
        <div className="mt-7 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
          <div className="bg-gradient-to-r from-gray-50 to-white px-5 py-4">
            ...
            <p className="text-xs text-gray-500">
              Choose add-ons to adjust premium instantly
            </p>
          </div>
        ...
        <div className="mt-7 grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
            <div className="bg-gradient-to-r from-gray-50 to-white px-5 py-4">
              ...
              <p className="text-xs text-gray-500">
                Refine quotes by coverage & price
              </p>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="grid grid-cols-1 gap-4">
              {quotes.map((q) => (
                <div key={q.quoteId} className="relative">
                  <div className="mb-2 flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => onAIRecommend(q)}
                      className="rounded-2xl bg-black px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90"
                    >
                      🤖 AI Recommendation
                    </button>
                  ...
                  <QuoteCard
                    ...
                    onView={(quote) =>
                      router.push(`/quotes?view=${quote.quoteId}`)
                    }
            ...
            {quotes.length === 0 && (
              <div className="rounded-3xl border border-gray-200 bg-white p-10 text-center shadow-sm">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Try resetting filters or increasing max premium.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl ring-1 ring-black/5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                {aiQuote && (
                  <p className="mt-1 text-sm text-gray-600">
                    {aiQuote.insurerName} • {aiQuote.planName}
                  </p>
                )}
              </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <button
                onClick={() => setAiOpen(false)}
                className="rounded-2xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-semibold text-gray-900 hover:bg-gray-50"
              >
                Close
              </button>
              ...
              {aiQuote && (
                <button
                  onClick={() => onSelectPlan(aiQuote)}
                  className="rounded-2xl bg-black px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90"
                >
                  Select this Plan →
                </button>
              )}
      ...
      {/* Premium Breakup Modal (URL driven) */}
      {viewBreakup && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-[2px] flex items-center justify-center px-4">
          <div className="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl ring-1 ring-black/5">
            ...
            <div className="mt-5 rounded-2xl bg-gray-50 p-4 ring-1 ring-gray-200">
              <div className="space-y-3 text-sm">
                ...
                <Row
                  label="Discount (NCB)"
                  value={`- ${formatINR(viewBreakup.discount)}`}
                  green
                />
            ...
            <button
              ...
              className="mt-5 w-full rounded-2xl bg-black px-4 py-3 text-white font-semibold shadow-sm transition hover:opacity-90 active:scale-[0.99]"
            >
              Continue →
            </button>
          </div>
